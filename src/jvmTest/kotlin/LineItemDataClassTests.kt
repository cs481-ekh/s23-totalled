...
class LineItemDataClassTests {
    ...
    fun lineItemNonTaxable_ZeroValueGiven_LineItemCreationSuccessful() {
        val item2 = LineItem(3.00, 0.00, "Purchase with No S+H", "2/15/2023", "Home Depot", CardType.JL, PurchaseType.PURCHASE)
        val item2ExpectedString = "LineItem(totalTaxable=3.0, totalNonTaxable=0.0, description=Purchase with No S+H, date=2/15/2023, vendor=Home Depot, cardType=JL, purchaseType=PURCHASE)"
    ...
    fun lineItemTaxableZero_ZeroValueGiven_LineItemCreationSuccessful() {
        val item2 = LineItem(0.00, 3.00, "Purchase with No S+H", "2/15/2023", "Home Depot", CardType.JL, PurchaseType.PURCHASE)
        val item2ExpectedString = "LineItem(totalTaxable=0.0, totalNonTaxable=3.0, description=Purchase with No S+H, date=2/15/2023, vendor=Home Depot, cardType=JL, purchaseType=PURCHASE)"
        assertEquals("No NonTax LineItem Creation Failed", item2ExpectedString, item2.toString())
    }
}
